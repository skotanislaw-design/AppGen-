...
- **NOMOS_AUDIT_API_KEY**: ορίστε τιμή στο `.env` ώστε τα endpoints
  ...
  Claude tokens και δεν πρέπει να είναι ανοιχτό στο internet. (Το frontend
  της παρούσας έκδοσης δεν στέλνει το header — για χρήση με κλειδί,
  περιορίστε εναλλακτικά την πρόσβαση σε επίπεδο nginx: `allow` για τα IP
  των γραφείων ή HTTP basic auth.)
